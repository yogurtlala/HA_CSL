...
def class_select(logits, target):
    ...
    batch_size, num_classes = logits.size()##bs*pn,2
    if target.is_cuda:
        ...
        one_hot_mask = torch.autograd.Variable(torch.arange(0, num_classes)
                                               ...
                                               .repeat(batch_size, 1)
                                               .eq(target.data.repeat(num_classes, 1).t()))
    return logits.masked_select(one_hot_mask)##只保留mask中为1的位置对应的值


def cross_entropy_with_weights(logits, target, weights=None):##输入是instance的预测,target是以bag_label为标签的instance
   ##weight是instance的权重，shape为bs*pn
    assert logits.dim() == 2
...
class CrossEntropyLoss(nn.Module):
    ...
    """
    def __init__(self, aggregate='mean'):##mean max有什么区别？
    ...
    def forward(self, input, target, weights=None):
        if self.aggregate == 'sum':
            ...
            return cross_entropy_with_weights(input, target, weights).mean()##return是1024,然后求均值
